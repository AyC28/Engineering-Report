...
Raspberry Pi 4B running Debian 13 was chosen instead to run the Phoenix6 framework. 
...
Data from the device will also be used to adjust current fed to the motors during the race to ensure the longevity of the battery throughout the entire race. By default, the current draw will be limited to a set amount as to not deplete the battery too fast. A switch will allow the driver to raise the current limit temporarily for actions such as overtaking, starting from a pit stop etc. The control software will then recalculate a new current limit that will preserve the battery for the duration of the race. 
